...
func TestRecombFraction(t *testing.T) {
	...
	if c := recombFraction(model, 0, 64, 1.0); math.Abs(c-0.5) > 1e-12 {
	...
	if c := recombFraction(model, 0, 10, 1.0); math.Abs(c-wantClose) > 1e-12 {
		t.Errorf("same-chromosome c = %v, want %v", c, wantClose)
	}

	// Monotone increasing with distance, saturating below 0.5.
	cNear := recombFraction(model, 0, 5, 1.0)
	cFar := recombFraction(model, 0, 40, 1.0)
