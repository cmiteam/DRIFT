...
func SetupPopFlood(model *core.Model, pop *core.Pop) error {
	...
	for couple := 0; couple < numCouples; couple++ {
		...
		// Create the man
		CreateFounder(pop, manID, 100, 650, FounderFitness(model), model, landCoordinates, 0)
		...
		// Create the woman
		CreateFounder(pop, womanID, 100, 650, FounderFitness(model), model, landCoordinates, 0)
