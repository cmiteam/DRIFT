...
func SetupPopCreation(model *core.Model, pop *core.Pop) error {
	...
	// Create Adam (male)
	adamID := 0
	CreateFounder(pop, adamID, 20, 900, FounderFitness(model), model, landCoordinates, 0)
	...
	// Create Eve (female)
	eveID := 1
	CreateFounder(pop, eveID, 20, 900, FounderFitness(model), model, landCoordinates, 0)
