...
def dead_string(ind, IndData, year):
    ...
    if ind in IndData:
        ...
        ind_info['Ygens'] = ind_info.get('Ygens', -1)
        ind_info['MTGens'] = ind_info.get('MTGens', -1)
        ind_info['MaxGenealGens'] = ind_info.get('MaxGenealGens', -1)
        ind_info['MinGenealGens'] = ind_info.get('MinGenealGens', -1)
        ind_info['MaxGeneticGens'] = ind_info.get('MaxGeneticGens', -1)
        ind_info['MinGeneticGens'] = ind_info.get('MinGeneticGens', -1)
        ind_info['seed_alleles'] = ind_info.get('seed_alleles', -1)
        ind_info['blocks'] = ind_info.get('blocks', -1)
        ind_info['CentromereCount'] = ind_info.get('CentromereCount', -1)
        ind_info['fitness'] = ind_info.get('fitness', -1)
        ind_info['NumMuts'] = ind_info.get('NumMuts', -1)
