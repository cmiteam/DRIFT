...
func Save(model *core.Model, pop *core.Pop, animManager *core.AnimationsContainer) {
	...
	if model.Parameters["track_SFS"] == 1 {
		sfsResult := necalcs.CalculateSFS(model, pop, false) // Use false for folded SFS
		numSegSitesSFS, singletons, doubletons, tajimasD, fuLiD, fayWuH = necalcs.GetSFSForSave(sfsResult)
		binnedSFS := necalcs.CalculateBinnedSFS(model, pop)
		sfsMean, sfsVar, sfsSkew, sfsExtremes = necalcs.GetSFSMoments(binnedSFS)
	...
	if model.Parameters["track_LD"] == 1 {
		ldSummary := ldanalysis.CalculateLDSummary(model, pop)
		ldShort, ldMedium, ldLong, highLDPairs, ldDecaySlope = ldanalysis.GetLDForSave(ldSummary)
	}
